...
1. Accessed [vSphere]() and selected the correct sandbox correlated to the Host IP address:
    ...
    <img src="https://github.com/davidfarcila/sysadmin-ticket-log/blob/main/Screenshots/T1_1.png?raw=true"/>
...
4. Named my resource pool as DARCILA-CLUSTER and clicked ‘OK’
...
5. Used the `id darcila` command to confirm if IPA was working correctly:
    
    
    
6. Used the `ipa user-show darcila` command to confirm the following:
    
    
    
7. **Note:** While configuring IPA if you get an error message, try: `ipa-client-install —-mkhomedir --force-join` or use `kinit darcila` to use Kerberos (security software used w/IPA to ensure connection is genuine) in order to confirm your access and identity.
</details>

<details>
<summary> 6. Add user bflores to FreeIPA </summary>
    
<code style="white-space: pre-wrap;">TASK: The developer's team recently hired Belle Flores. Please create a username bflores for the new user and add them to the group "webmasters." Additionally, set up a temporary password for the user and ensure this information is documented in the ticket.</code>
    
**STEPS TAKEN:**
    
1. Logged into freeIPA, added a new user for Belle Flores, set password and clicked “Add and Edit”
    
    
    
2. Clicked on the “User Groups” tab in order to add Belle Flores to the group “Webmasters” by selecting it and moving it from “Available” to “Prospective,” then clicked “Add”
    
    
    
3. Confirmed user was created by running the `id bflores` command:
...
1. Clicked on “Active Users” and searched for bflores
...
3. Confirmed user was added to the group by running the `id bflores` command:
